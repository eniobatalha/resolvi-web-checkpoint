...
export default function ProfileClient() {
  ...
  const [editedData, setEditedData] = useState<Partial<ClientData> & { address: AddressEntity }>({
  ...
  useEffect(() => {
    const fetchClientData = async () => {
      try {
        ...
        const response = await axiosInstance.get<ClientData>(`/api/client/${id}`);
  ...
  const handleSaveInfo = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.patch(
        `/api/client/update/${clientData.id}`,
        payload
      );
  ...
  const handleSaveAddress = async () => {
    ...
    try {
      ...
      const endpoint = `/api/client/address/${clientData.id}`;
      const method = clientData.address ? 'patch' : 'post';

      const response = await axiosInstance[method]<ClientData>(
        endpoint,
        addressPayload
      );
  ...
  if (!clientData) return <div>Cliente não encontrado</div>;
    ...
    <main className="flex-1 bg-white overflow-y-auto flex">
      ...
      <div className="w-1/3 p-10 flex flex-col items-center space-y-4">
        <Avatar className="w-40 h-40 mb-4">
          <AvatarImage src={clientData.profilePic || "https://via.placeholder.com/150"} />
          <AvatarFallback>{clientData.name[0]}</AvatarFallback>
        </Avatar>

        <h2 className="pt-4 font-bold text-[32px] text-indigo-900">{clientData.name}</h2>
        <p className="text-xl pb-1 text-gray-600">{clientData.email}</p>
        <p className="text-xl pb-1 text-gray-600">{clientData.phone}</p>
        {clientData.cpf && <p className="text-xl pb-4 text-gray-600">CPF: {clientData.cpf}</p>}
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4">
            <div className="bg-white p-6 rounded-lg w-full max-w-md">
              ...
              <FieldContainer
                ...
                editMode={true}
                onChange={(value) => setEditedData({ ...editedData, cpf: value })}
              />
              <FieldContainer
